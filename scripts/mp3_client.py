#!/usr/bin/python

import socket
import sys
import datetime as dt
import time as t
import threading

import simplejson as json

def print_error(error):
	print "Error %d: %s" % (error.args[0], error.args[1])

def get_timestamp():
	return dt.datetime.fromtimestamp(int(t.time())).strftime('%Y-%m-%d %H:%M:%S')

def raichu_send(out_sock, out_data):
	total_sent = 0
	while total_sent < len(out_data):
		try:
			print "sent: " + out_data[total_sent:]
			sent = out_sock.send(out_data[total_sent:])
			if sent == 0:
				raise RuntimeError("socket connection broken")
			total_sent += sent
		except socket.error, e:
			print_error(e)

def recv_data(in_sock):
	while True:
		in_data = in_sock.recv(256)
		if in_data == '':
			sys.exit(1)
			break
		print ">>>>> " + in_data

def raichu_recv(in_sock, buf_size):
	in_data = in_sock.recv(int(buf_size))
	if in_data == '':
		raise RuntimeError("socket connection broken")
	else:
		return in_data

if __name__ == "__main__":
	s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	host = sys.argv[1]
	port = sys.argv[2]
	cmd_history = []

	device_info = {
		'type'		  : 'client',
		'name'		  : 'client_' + dt.datetime.fromtimestamp(int(t.time())).strftime('%H%M%S'),
		'bootup_time' : get_timestamp(),
	}
	recv_worker = threading.Thread(target=recv_data, args=(s,),)
	recv_worker.daemon = True
	

	try:
		s.connect ((host, int(port)))
		out_data = json.dumps(device_info)
		recv_worker.start()
		raichu_send(s, out_data)
		print device_info["name"] + " start up"

		t.sleep(1)
		
		raichu_send(s, "connect mp3player")

		t.sleep(1)
		while True:
			buf = raw_input("> ")
			#cmd_history.append(buf)
			raichu_send(s, "relay " + buf)
			t.sleep(0.1)

	except socket.error, e:
		print_error(e)
	except KeyboardInterrupt:
		pass
	finally:
		if s:
			s.close()
		print "shutting down..."
		sys.exit(0)
